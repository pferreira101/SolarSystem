...
vector<Point> getFilePoints(const string& name, std::vector<Point>* nms);
...
Figure getFigure(const string& name){
	...
	vector<Point> points = getFilePoints(name, &normals);
	f.set_values(points, normals);
	return f;
}
...
*/
vector<Point> getFilePoints(const string& name, std::vector<Point>* normals) {
	string point;
	ifstream file;
	file.open(name);
